...
function signBlob (key, blob) {
  return crypto.createHmac('sha256', key).update(blob).digest('hex')
}
...
function mkReq (url, method) {
  ...
  req.url = url
  req.headers = {
      'x-gogs-signature'   : 'bogus'
    , 'x-gogs-event'    : 'bogus'
    , 'x-gogs-delivery' : 'bogus'
  }
  return req
}
...
test('handler accepts a signed blob', function (t) {
  ...
  req.headers['x-gogs-signature'] = signBlob('bogus', json)
  req.headers['x-gogs-event']  = 'push'
...
test('handler accepts a signed blob with alt event', function (t) {
  ...
  req.headers['x-gogs-signature'] = signBlob('bogus', json)
  req.headers['x-gogs-event']  = 'issue'
...
test('handler rejects a badly signed blob', function (t) {
  ...
  req.headers['x-gogs-signature'] = '0' + req.headers['x-gogs-signature'].substring(1)
  ...
  h.on('error', function (err, _req) {
    ...
    t.deepEqual(res.$headers, { 'content-type': 'application/json' })
    t.equal(res.$end, '{"error":"x-gogs-signature does not match blob signature"}', 'got correct content')
  })
